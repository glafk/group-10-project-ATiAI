...
def eval_pipnet(net,
    ...
    save_pooled = torch.empty([1, net.module._classification.weight.data.size()[1]]).to(device)
    ys_agg = []
    # 
...
#           
            #
            pooled_copy = pooled.clone()
            save_pooled = torch.cat([save_pooled, pooled_copy])
            ys_agg.extend(ys.cpu().tolist())
            #
    ...
    df.to_csv(f"outfile_{datetime.now().strftime('%Y-%m-%d-%H:%M:%S')}",index=False) #save to file
    ...
    df['labels'] = ys_agg
    df.to_csv(f"testfile_{datetime.now().strftime('%Y-%m-%d-%H:%M:%S')}_random_forest",index=False) #save to file
